...
class BCEFocalLoss(torch.nn.Module):
    def __init__(self, gamma=0, alpha=10, reduction='elementwise_mean'): 
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha
...
def save_test_result(test_log_dir, total_test_pre, total_test_f1, total_test_rec, total_test_spc, total_test_auc, \
    ...
    opt thresh 二值化， 保存每个fold 的各项指标至 {test_log_dir}test_threshopt_block_result.csv
    '''
    #####################################
    ...
    ## save fold info 
    ######################
    total_test_pre = np.array(total_test_pre)
    total_test_f1 = np.array(total_test_f1)
    total_test_rec = np.array(total_test_rec)
    total_test_auc = np.array(total_test_auc)
    total_test_acc = np.array(total_test_acc)
    total_opt_acc = np.array(total_opt_acc)
    total_opt_thresh = np.array(total_opt_thresh)
    total_opt_point = np.array(total_opt_point)

    fold_pred_info_normalthresh = np.stack((total_test_auc, total_test_acc, total_test_pre, total_test_rec, total_test_f1, total_test_spc, total_test_tp, total_test_fp, total_test_fn, total_test_tn), axis=0)
    ...
    fold_pred_info_optthresh = np.stack((total_test_auc, total_opt_acc, total_opt_pre, total_opt_rec, total_opt_f1, total_opt_spc, total_opt_tp, total_opt_fp, total_opt_fn, total_opt_tn, total_opt_thresh), axis=0)
    df3 = pd.DataFrame(data=fold_pred_info_optthresh, columns=['indicator', 'fold0', 'fold1', 'fold2', 'fold3', 'fold4'])
    df3.to_csv(f'{test_log_dir}test_threshopt_block_result.csv', index=False)
