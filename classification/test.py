...
parser.add_argument('--root_path', type=str, default='/mnt/ExtData/workspace/pahsos/Data/preprocessed', help='original data root')
parser.add_argument('--json_path', type=str, default='/mnt/ExtData/workspace/pahsos/Data/data_split.json', help='json path for data and label loading')
parser.add_argument('--samples_per_patient', type=int, default=12, help='sampling x blocks for one person') 

parser.add_argument('--log_dir', type=str, default='/mnt/ExtData/workspace/pahsos/classification/log', help='original data root')
...
## data_list 
ori_patient_list, ori_label_list = get_data_list(args, key='train')  # 可用于验证集结果复现，由于目前暂不需要，这里都暂时不运行
